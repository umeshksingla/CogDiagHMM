...
class SeasonsFwdTaskData(BaseData):
    """
    ...
    def get_inputs_array(self, n_len):
        ...
        stim_seq = np.random.choice([0, 1, 2, 3, 4, 5], p=[0.9, 0.02, 0.02, 0.02, 0.02, 0.02], size=n_len)
        # stim_seq = np.random.randint(0, self.vocab_size, (n_len,))
...
def execute():
    ...
    gen_model = SeasonsFwdTaskData(N_STATES, N_INPUTS, N_OBS_DIM)
