...
def plot_gaussian_hmm_data2(model, emissions, predictions, states, xlim=None, title=None, suffix=None, savefig=None, fig_dir=None, display=False):
    ...
    plt.tight_layout()
    if savefig: fig.savefig(os.path.join(fig_dir, f'{suffix}_emissions_true_vs_pred.pdf'), bbox_inches='tight', dpi=300,
                            transparent=True)
    if display: plt.show()
    plt.close()
    return
...
def plot_ethogram(transition_matrix, savefig=False, fig_dir=None, display=True):
    ...
    for i in range(num_states):
        for j in range(num_states):
            if round(transition_matrix[i, j], 2) > 0.001:  # Only add edges with nonzero probability
...
def plot_loss(em_losses, savefig=False, fig_dir=None, display=True):
    ...
    plt.margins(0.2)
    plt.tight_layout()
    if savefig: fig.savefig(os.path.join(fig_dir, 'loss.pdf'), bbox_inches='tight', dpi=300, transparent=True)
    if display: plt.show()
    plt.close()
    return
...
def plot_state_o_dists(emissions_z, emission_dim, title=None, suffix='', savefig=False, fig_dir=None, display=True):
    ...
    for o in range(emission_dim):
        ...
        for z in list(emissions_z.keys()):
            ...
            #             # clip=(x0, x99)
            #             )
            sns.histplot(data, color=COLORS[z], stat='proportion', binwidth=0.1, ax=ax, label=f'State {z+1}')
...
def plot_state_durations(model, state_probs, suffix='', savefig=False, fig_dir=None, display=True):
...
def plot_state_possible_emission_vectors(model, emission_dim, savefig=False, fig_dir=None, display=True):
    W = model.learned_params.emissions.weights
    B = model.learned_params.emissions.biases
